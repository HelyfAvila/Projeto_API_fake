...
const CartItem = ({item}) => {
  ...
  return (
    <div className='flex gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full front-light text-gray-500'>
      <div className='w-full min-h-[150px] flex items-center gap-x-4'>
        {/* image */}
        <Link to={`/products/${id}`}>
          <img className='max-w-[80px]' src={images} alt=''></img>
        </Link>
        <div className='w-full flex flex-col'>
          ...
          <div className='flex gap-x-2 h-[36px] text-sm'>
            ...
            <div className='flex flex-1 max-w-[100px] items-center h-full border text-primary font-medium'>
              {/* minus icon */}
              <div 
              onClick={() => decreaseAmount(id)} className='flex-1 flex justify-center items-center cursor-pointer h-full'>
                <IoMdRemove / >
              </div>
              {/* amount */}
              ...
              {/* pluss icon */}
              <div 
              onClick={()=> increaseAmount(id)} className='flex-1 h-full flex justify-center items-center cursor-pointer'>
                <IoMdAdd / >
              </div>
            </div>
            {/* Preço item  */}
